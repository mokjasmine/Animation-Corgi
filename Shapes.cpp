...
point4 triPoints[numTriangleVertices];
point3 triNormals[numTriangleVertices];
point2 triUV[numTriangleVertices];

point4 triVert[3] = {
    point4(-1.0f, 0.5f, 0.0f, 1.0),
    point4(-1.0f, -0.5f, 0.0f, 1.0),
    point4(0.0f, 0.0f, 0.0f, 1.0)
};


void quad(int a, int b, int c, const point3& normal)
{
    int in=0;
    triPoints[in] = triVert[a];
    triNormals[in] = -cross((triVert[a]-triVert[b]), (triVert[a]-triVert[c])); // flat phong: put cross product as normal. make sure cross product isn't pointing in
    triUV[in] = point2(0.0f, 1.0f);
    in++;
    triPoints[in] = triVert[b];
    triNormals[in] = -cross((triVert[a]-triVert[b]), (triVert[a]-triVert[c]));
    triUV[in] = point2(0.0f, 0.0f);
    in++;
    triPoints[in] = triVert[c];
    triNormals[in] = -cross((triVert[a]-triVert[b]), (triVert[a]-triVert[c]));
    triUV[in] = point2(1.0f, 0.0f);
    in++;
}

void colorTri()
{
    quad(0, 1, 2, point3(0.0f, 0.0f, -1.0f));
}

void generateTriangle(GLuint program, ShapeData* tridata)
{
    colorTri();
    tridata->numVertices = numTriangleVertices;
    glGenVertexArrays(1, &tridata->vao);
    glBindVertexArray(tridata->vao);
    
    setVertexAttrib(program, (float*)triPoints, sizeof(triPoints), (float*)triNormals, sizeof(triNormals), 0, 0);
}

//--------------------------------------------------------------------------
//left triangle

const int numTriangleVertices2 = 3;
point4 triPoints2[numTriangleVertices2];
point3 triNormals2[numTriangleVertices2];
point2 triUV2[numTriangleVertices2];

point4 triVert2[5] = {
    point4(0.0f, 0.0f, 0.0f, 1.0),
    point4(1.0f, 0.5f, 0.4f, 1.0),
    point4(1.0f, -0.5f, 0.4f, 1.0)
};


void quad2(int a, int b, int c, const point3& normal)
{
    int in=0;
    triPoints2[in] = triVert2[a];
    triNormals2[in] = -cross((triVert[a]-triVert[b]), (triVert[a]-triVert[c]));
    triUV2[in] = point2(0.0f, 1.0f);
    in++;
    triPoints2[in] = triVert2[b];
    triNormals2[in] = -cross((triVert[a]-triVert[b]), (triVert[a]-triVert[c]));
    triUV2[in] = point2(0.0f, 0.0f);
    in++;
    triPoints2[in] = triVert2[c];
    triNormals2[in] = -cross((triVert[a]-triVert[b]), (triVert[a]-triVert[c]));
    triUV2[in] = point2(1.0f, 0.0f);
    in++;
}

void colorTri2()
{
    quad2(0, 1, 2, point3(0.0f, 0.0f, -1.0f));
}

void generateTriangle2(GLuint program, ShapeData* tridata2)
{
    colorTri2();
    tridata2->numVertices = numTriangleVertices2;
    glGenVertexArrays(1, &tridata2->vao);
    glBindVertexArray(tridata2->vao);
    
    setVertexAttrib(program, (float*)triPoints2, sizeof(triPoints2), (float*)triNormals2, sizeof(triNormals2), 0, 0);
}
...
{
    generateTriangle(program, tridata);
    generateTriangle2(program, tridata2);
}
